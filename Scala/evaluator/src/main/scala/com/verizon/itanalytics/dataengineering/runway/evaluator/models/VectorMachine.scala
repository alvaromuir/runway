...
import com.verizon.itanalytics.dataengineering.runway.evaluator.models.elements.SupportVectorMachineModel
...
trait VectorMachine extends SupportVectorMachineModel {

  /** Parses provided pMML file as an Support Vector Machine Model
    *
    * @param pMML a valid pMML file
    * @return SupportVectorMachineModel
    */
  def parseSupportVectorMachineModel(pMML: PMML): SupportVectorMachineModel = {
    ...
    SupportVectorMachineModel(
          ...
          Option(
            ModelExplanation(
              extension = model.getModelExplanation.getExtensions match {
                case null => None
                case _ =>
                  Option(model.getModelExplanation.getExtensions.asScala.map {
                    e =>
                      Extension(
                        extender = e.getExtender match {
                          case null => None
          ...
          Option(
            LocalTransformation(
              extension = model.getLocalTransformations.getExtensions match {
                ...
                case _ =>
                  Option(
                    model.getLocalTransformations.getExtensions.asScala.map {
                      e =>
                        Extension(
                          extender = e.getExtender match {
                            case null => None
      ...
      kernel = model.getKernel.getClass.getSimpleName,
      vectorDictionary = VectorDictionary(
        extension = model.getVectorDictionary.getExtensions match {
          case null => None
          case _ =>
            Option(model.getVectorDictionary.getExtensions.asScala.map {
              e =>
                Extension(
                  extender = e.getExtender match {
                    case null => None
        ...
        vectorFields = VectorFields(
          extension = model.getVectorDictionary.getVectorFields.getExtensions match {
            case null => None
            case _ =>
              Option(model.getVectorDictionary.getVectorFields.getExtensions.asScala.map {
                e =>
                  Extension(
                    extender = e.getExtender match {
                      case null => None
          ...
          numberOfFields = model.getVectorDictionary.getVectorFields.getNumberOfFields match {
            case null => None
            case _ => Option(model.getVectorDictionary.getVectorFields.getNumberOfFields.toInt)
          },
          content = model.getVectorDictionary.getVectorFields.getContent match {
            case null => None
            case _ => Option(model.getVectorDictionary.getVectorFields.getContent.asScala.map {
              c => Content(
        ...
        vectorInstance = model.getVectorDictionary.getVectorInstances match {
          case null => None
          case _ =>
            Option(model.getVectorDictionary.getVectorInstances.asScala.map {
              i =>
                VectorInstance(
                  extension = i.getExtensions match {
                    case null => None
                    case _ =>
                      Option(i.getExtensions.asScala.map {
                        e =>
                          Extension(
                            extender = e.getExtender match {
                              case null => None
                  ...
                  key = i.getKey match {
                    case null => None
                    case _    => Option(i.getKey)
                  },
                  array = i.getArray match {
                    case null => None
                    case _ =>
                      Option(
                        Array(
                          n = i.getArray.getN.toInt,
                          `type` = i.getArray.getType.toString,
                          value = i.getArray.getValue
                        ))
                  },
                  realSparseArray = i.getRealSparseArray match {
                    case null => None
                    case _ =>
                      Option(
                        RealSparseArray(
                          indices = i.getRealSparseArray.getIndices.asScala.map { _.toDouble },
                          entries = i.getRealSparseArray.getEntries.asScala.map { _.toDouble },
                          n = i.getRealSparseArray.getN.toInt,
                          defaultValue = i.getRealSparseArray.getDefaultValue match {
                              case null => None
                              case _ =>
                                Option(
                                  i.getRealSparseArray.getDefaultValue.toDouble)
                            }
                        ))
                  }
                )
        ...
        numberOfVectors = model.getVectorDictionary.getNumberOfVectors match {
          case null => None
          case _    => Option(model.getVectorDictionary.getNumberOfVectors.toInt)
        }
      ),
      supportVectorMachine = model.getSupportVectorMachines.asScala.map { v =>
        SupportVectorMachine(
          extension = v.getExtensions match {
            case null => None
            case _ =>
              Option(v.getExtensions.asScala.map {
                e =>
                  Extension(
                    extender = e.getExtender match {
                      case null => None
          ...
          supportVectors = v.getSupportVectors match {
            case null => None
            case _ =>
              Option(
                SupportVectors(
                  extension = v.getSupportVectors.getExtensions match {
                    case null => None
                    case _ =>
                      Option(v.getSupportVectors.getExtensions.asScala.map {
                        e =>
                          Extension(
                            extender = e.getExtender match {
                              case null => None
                  ...
                  supportVector = v.getSupportVectors.asScala.map {
                    s =>
                      SupportVector(
                        ...
                        vectorId = s.getVectorId
                      )
                  },
                  numberOfSupportVectors =
                    v.getSupportVectors.getNumberOfSupportVectors match {
                      case null => None
                      case _ =>
                        Option(
                          v.getSupportVectors.getNumberOfSupportVectors.toInt)
                    },
                  numberOfAttributes = v.getSupportVectors match {
                    case null => None
                    case _ =>
                      Option(v.getSupportVectors.getNumberOfAttributes.toInt)
                  }
                ))
          },
          coefficients = Coefficients(
            extension = v.getCoefficients.getExtensions match {
              case null => None
              case _ =>
                Option(v.getCoefficients.getExtensions.asScala.map {
                  e =>
                    Extension(
                      extender = e.getExtender match {
                        case null => None
            ...
            coefficient = v.getCoefficients.asScala.map { c =>
              Coefficient(
                extension = c.getExtensions match {
                  case null => None
                  case _ =>
                    Option(c.getExtensions.asScala.map {
                      e =>
                        Extension(
                          extender = e.getExtender match {
                            case null => None
                ...
                value = c.getValue match {
                  case null => None
                  case _    => Option(c.getValue.toDouble)
                }
              )
            },
            numberOfCoefficients =
              v.getCoefficients.getNumberOfCoefficients match {
                case null => None
                case _ =>
                  Option(v.getCoefficients.getNumberOfCoefficients.toInt)
              },
            absoluteValue = v.getCoefficients.getAbsoluteValue match {
              case null => None
              case _    => Option(v.getCoefficients.getAbsoluteValue.toDouble)
            }
          ),
          targetCategory = v.getTargetCategory match {
            case null => None
            case _    => Option(v.getTargetCategory)
          },
          alternateTargetCategory = v.getAlternateTargetCategory match {
            case null => None
            case _    => Option(v.getAlternateTargetCategory)
          },
          threshold = v.getThreshold match {
            case null => None
            case _    => Option(v.getThreshold.toDouble)
          }
        )
      },
      ...
      threshold = model.getThreshold match {
        case null => None
        case _    => Option(model.getThreshold.toDouble)
      },
      svmRepresentation = model.getRepresentation match {
        case null => None
        case _    => Option(model.getRepresentation.value())
      },
      classificationMethod = model.getClassificationMethod match {
        case null => None
        case _    => Option(model.getClassificationMethod.value())
      },
      alternateBinaryTargetCategory = model.getAlternateBinaryTargetCategory match {
        case null => None
        case _ => Option(model.getAlternateBinaryTargetCategory)
      },
      maxWins = Option(model.isMaxWins),
