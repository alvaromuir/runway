...
import com.verizon.itanalytics.dataengineering.runway.evaluator.models.GaussianProcess
import org.dmg.pmml.PMML

import scala.collection.JavaConverters._

trait GaussianProcessParser extends GaussianProcess {
  ...
  def parseGaussianProcessModel(pMML: PMML): GaussianProcess = {
    ...
    GaussianProcess(
      ...
      optimizer = gaussianProcessModel match {
        case null => None
      ...
      miningSchema = MiningSchema(miningFields =
        gaussianProcessModel.getMiningSchema.getMiningFields match {
          case null => None
          case _ =>
            Option(
      ...
      output = gaussianProcessModel.getOutput match {
        ...
        case _ =>
          Option(
            Output(
              outputFields =
                gaussianProcessModel.getOutput.getOutputFields.asScala
                  .map {
                    o =>
                      OutputField(
      ...
      modelExplanation = gaussianProcessModel.getModelExplanation match {
        case null => None
        case _ =>
          val me = gaussianProcessModel.getModelExplanation
          Option(
            ModelExplanation(
      ...
      localTransformation = gaussianProcessModel.getLocalTransformations match {
        case null => None
        case _ =>
          Option(LocalTransformation(derivedFields =
            gaussianProcessModel.getLocalTransformations.getDerivedFields match {
              case null => None
              case _ =>
                Option(
