...
trait Regression extends RegressionModel {
  ...
  /** Parses provided pMML file as an Regression Model
    *
      ...
      regressionTable = regressionModel.getRegressionTables.asScala.map { t =>
        RegressionTable(
          extension = t.getExtensions match {
            case null => None
            case _ =>
              Option(t.getExtensions.asScala.map {
                e =>
                  Extension(
                    extender = e.getExtender match {
                      case null => None
          ...
          numericPredictor = t.getNumericPredictors match {
            case null => None
            case _ =>
              Option(t.getNumericPredictors.asScala.map { p =>
                NumericPredictor(
                  extension = p.getExtensions match {
                    case null => None
                    case _ =>
                      Option(t.getExtensions.asScala.map {
                        e =>
                          Extension(
                            ...
                            content = e.getContent match {
                              case null => None
                              case _ => Option(e.getContent.asScala.map {
                                _.toString
                              })
                            }
                          )
                  ...
                  name = p.getName match {
                  case null => None
                  case _    => Option(p.getName.getValue)
                }, exponent = p.getExponent.toInt
                  , coefficient = p.getCoefficient)
              })
          },
          categoricalPredictor = t.getCategoricalPredictors match {
            case null => None
            case _ =>
              Option(t.getCategoricalPredictors.asScala.map { p =>
                CategoricalPredictor(extension = p.getExtensions match {
                  case null => None
                  case _ =>
                    Option(t.getExtensions.asScala.map {
                      e =>
          ...
          predictorTerm = t.getPredictorTerms match {
            case null => None
            case _ =>
              Option(t.getPredictorTerms.asScala.map { p =>
                PredictorTerm(extension = p.getExtensions match {
                  case null => None
                  case _ =>
                    Option(t.getExtensions.asScala.map {
                      e =>
          ...
          intercept = t.getIntercept,
          targetCategory = t.getTargetCategory match {
            case null => None
            case _    => Option(t.getTargetCategory)
          }
        )
      },
