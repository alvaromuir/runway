...
import com.verizon.itanalytics.dataengineering.runway.evaluator.models.elements.TextModel
...
trait TextModels extends TextModel {
  ...
  def parseTextModel(pMML: PMML): TextModel = {
    val model = pMML.getModels
      .get(0)
      .asInstanceOf[org.dmg.pmml.text.TextModel]
    ...
    TextModel(
      ...
      textDictionary = TextDictionary(
        extension = model.getTextDictionary.getExtensions match {
          case null => None
          case _ =>
            Option(model.getTextDictionary.getExtensions.asScala.map {
              e =>
                Extension(
                  extender = e.getExtender match {
                    case null => None
        ...
        taxonomy = model.getTextDictionary.getTaxonomy match {
          case null => None
          case _ => Option(Taxonomy(
            extension = model.getTextDictionary.getTaxonomy.getExtensions match {
              case null => None
              case _ => Option(model.getTextDictionary.getTaxonomy.getExtensions.asScala.map {
                  e =>
                    Extension(
                      extender = e.getExtender match {
                        case null => None
            ...
            name = model.getTextDictionary.getTaxonomy.getName,
            childParents = model.getTextDictionary.getTaxonomy.getChildParents match {
              case null => None
              case _ => Option(model.getTextDictionary.getTaxonomy.getChildParents.asScala.map {
                cp => ChildParent(
                  extension = cp.getExtensions match {
                    case null => None
                    case _=>
                      Option(cp.getExtensions.asScala.map {
                        e =>
                          Extension(
                            extender = e.getExtender match {
                              case null => None
                  ...
                  parentLevelField = cp.getParentLevelField match {
                    case null => None
                    case _ => Option(cp.getParentField)
                  },
                  isRecursive = cp.getRecursive.value(),
      ...
      textCorpus = TextCorpus(
        extension = model.getTextCorpus.getExtensions match {
          case null => None
          case _ => Option(model.getTextCorpus.getExtensions.asScala.map {
            e =>
              Extension(
                extender = e.getExtender match {
                  case null => None
          ...
          textDocument = model.getTextCorpus.getTextDocuments match {
            case null => None
            case _ => Option(model.getTextCorpus.getTextDocuments.asScala.map {
              d => TextDocument(
                ...
                name = d.getName match {
                  case null => None
                  case _ => Option(d.getName)
                },
                length = d.getLength match {
                  case null => None
                  case _ => Option(d.getLength)
                },
                file = d.getFile match {
                  case null => None
                  case _ => Option(d.getFile)
                }
              )
      ...
      documentTermMatrix = DocumentTermMatrix(
        extension = model.getDocumentTermMatrix.getExtensions match {
          case null => None
          case _ =>
            Option(model.getDocumentTermMatrix.getExtensions.asScala.map {
              e =>
                Extension(
                  extender = e.getExtender match {
                    case null => None
        ...
        matrix = Matrix(
          matCell = model.getDocumentTermMatrix.getMatrix.getMatCells match {
            case null => None
            case _ => Option(model.getDocumentTermMatrix.getMatrix.getMatCells.asScala.map {
              c => MatCell(
                row = c.getRow,
                col = c.getCol,
                value = c.getValue
          ...
          kind = model.getDocumentTermMatrix.getMatrix.getKind.value(),
          nbRows = model.getDocumentTermMatrix.getMatrix.getNbRows match {
            case null => None
            case _ => Option(model.getDocumentTermMatrix.getMatrix.getNbRows.toInt)
          },
          nbCols = model.getDocumentTermMatrix.getMatrix.getNbCols match {
            case null => None
            case _ => Option(model.getDocumentTermMatrix.getMatrix.getNbCols.toInt)
          },
          diagDefault = model.getDocumentTermMatrix.getMatrix.getDiagDefault match {
            case null => None
            case _ => Option(model.getDocumentTermMatrix.getMatrix.getDiagDefault.toDouble)
          },
          offDiagDefault = model.getDocumentTermMatrix.getMatrix.getOffDiagDefault match {
          case null => None
          case _ => Option(model.getDocumentTermMatrix.getMatrix.getOffDiagDefault.toDouble)
        }
        )
      ),
      textModelNormalization = model.getTextModelNormalization match {
        case null => None
        case _ => Option(TextModelNormalization(
          extension = model.getTextModelNormalization.getExtensions match {
            case null => None
            case _ =>
              Option(model.getTextModelNormalization.getExtensions.asScala.map {
                e =>
                  Extension(
                    extender = e.getExtender match {
                      case null => None
          ...
          localTermWeights = model.getTextModelNormalization.getLocalTermWeights match {
            case null => None
            case _ => Option(model.getTextModelNormalization.getLocalTermWeights.value())
          },
          globalTermWeights = model.getTextModelNormalization.getGlobalTermWeights match {
            case null => None
            case _ => Option(model.getTextModelNormalization.getGlobalTermWeights.value())
          },
          documentNormalization = model.getTextModelNormalization.getDocumentNormalization match {
            case null => None
            case _ => Option(model.getTextModelNormalization.getDocumentNormalization.value())
          }
        ))
      },
      textModelSimiliarity = model.getTextModelSimiliarity match {
        case null => None
        case _ => Option(TextModelSimiliarity(
          extension = model.getTextModelSimiliarity.getExtensions match {
            case null => None
            case _ =>
              Option(model.getTextModelSimiliarity.getExtensions.asScala.map {
                e =>
                  Extension(
                    extender = e.getExtender match {
                      case null => None
          ...
          similarityType = model.getTextModelSimiliarity.getSimilarityType match {
            case null => None
            case _ => Option(model.getTextModelSimiliarity.getSimilarityType.value())
          }
        ))
      },
      ...
      numberOfTerms = model.getNumberOfTerms,
      numberOfDocuments = model.getNumberOfDocuments,
