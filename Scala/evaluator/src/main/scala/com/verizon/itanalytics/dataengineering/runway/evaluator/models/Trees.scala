...
import com.verizon.itanalytics.dataengineering.runway.evaluator.models.elements.TreeModel
...
trait Trees extends TreeModel {

  /** Parses provided pMML file as an Tree Model
    *
    * @param pMML a valid pMML file
    * @return TreeModel
    */
  def parseTreeModel(pMML: PMML): TreeModel = {
    val model = pMML.getModels
      .get(0)
      .asInstanceOf[org.dmg.pmml.tree.TreeModel]
  ...
  def parseNode(n: org.dmg.pmml.tree.Node): Node = {
    Node(
      ...
      predicate = n.getPredicate.getClass.getSimpleName,
      partition = n.getPartition match {
        case null => None
        case _ => Option(Partition(
          extension = n.getPartition.getExtensions match {
            case null => None
            case _ =>
              Option(n.getPartition.getExtensions.asScala.map {
                e =>
                  Extension(
                    extender = e.getExtender match {
                      case null => None
          ...
          partitionFieldStats = n.getPartition.getPartitionFieldStats match {
            case null => None
            case _ =>
              Option(n.getPartition.getPartitionFieldStats.asScala.map {
                s =>
                  PartitionFieldStats(
                          ...
                          e =>
                            Extension(
                              ...
                              content = e.getContent match {
                                case null => None
                                case _ =>
                                  Option(e.getContent.asScala.map { _.toString })
                              }
                            )
                        })
                    },
                    counts = s.getCounts match {
                      case null => None
                      case _ =>
                        Option(
                          Counts(
                            extension = s.getCounts.getExtensions match {
                              case null => None
                              case _ =>
                                Option(s.getCounts.getExtensions.asScala.map {
                                  e =>
                                    Extension(
                                      extender = e.getExtender match {
                                        case null => None
                            ...
                            totalFreq = s.getCounts.getTotalFreq,
                            missingFreq = s.getCounts.getMissingFreq match {
                              case null => None
                              case _ =>
                                Option(s.getCounts.getMissingFreq.toDouble)
                            },
                            invalidFreq = s.getCounts.getInvalidFreq match {
                              case null => None
                              case _ =>
                                Option(s.getCounts.getInvalidFreq.toDouble)
                            },
                            cardinality = s.getCounts.getCardinality match {
                              case null => None
                              case _    => Option(s.getCounts.getCardinality.toInt)
                            }
                          ))
                    },
                    numericInfo = s.getNumericInfo match {
                      case null => None
                      case _ =>
                        Option(
                          NumericInfo(
                            extension = s.getNumericInfo.getExtensions match {
                              case null => None
                              case _ =>
                                Option(
                                  s.getNumericInfo.getExtensions.asScala.map {
                                    e =>
                                      Extension(
                                        extender = e.getExtender match {
                                          case null => None
                            ...
                            quantiles = s.getNumericInfo.getQuantiles match {
                              case null => None
                              case _ =>
                                Option(s.getNumericInfo.getQuantiles.asScala.map {
                                  q =>
                                    Quantile(
                                      extension = q.getExtensions match {
                                        case null => None
                                        case _ =>
                                          Option(q.getExtensions.asScala.map {
                                            e =>
                                              Extension(
                                                extender = e.getExtender match {
                                                  case null => None
                            ...
                            minimum = s.getNumericInfo.getMinimum match {
                              case null => None
                              case _ =>
                                Option(s.getNumericInfo.getMinimum.toDouble)
                            },
                            maximum = s.getNumericInfo.getMaximum match {
                              case null => None
                              case _ =>
                                Option(s.getNumericInfo.getMaximum.toDouble)
                            },
                            mean = s.getNumericInfo.getMean match {
                              case null => None
                              case _    => Option(s.getNumericInfo.getMean.toDouble)
                            },
                            standardDeviation =
                              s.getNumericInfo.getMaximum match {
                                case null => None
                                case _ =>
                                  Option(s.getNumericInfo.getMaximum.toDouble)
                              },
                            median = s.getNumericInfo.getMedian match {
                              case null => None
                              case _ =>
                                Option(s.getNumericInfo.getMedian.toDouble)
                            },
                            interQuartileRange =
                              s.getNumericInfo.getInterQuartileRange match {
                                case null => None
                                case _ =>
                                  Option(
                                    s.getNumericInfo.getInterQuartileRange.toDouble)
                              }
                          ))
                    },
                    field = s.getField.getValue,
                    weighted = s.getWeighted match {
                      case null => Option("0")
                      case _    => Option(s.getWeighted.value())
                    }
                  )
              })
          },
          name = n.getPartition.getName,
          size = n.getPartition.getSize match {
            case null => None
            case _    => Option(n.getPartition.getSize.toDouble)
          }
        ))
      },
      scoreDistributions = n.getScoreDistributions match {
        case null => None
        case _ =>
          Option(n.getScoreDistributions.asScala.map {
            d =>
              ScoreDistribution(
                extension = d.getExtensions match {
                  case null => None
                  case _ =>
                    Option(d.getExtensions.asScala.map {
                      e =>
                        Extension(
                          extender = e.getExtender match {
                            case null => None
                ...
                confidence = d.getConfidence match {
                  case null => None
                  case _    => Option(d.getConfidence.toDouble)
                },
                probability = d.getProbability match {
                  case null => None
                  case _    => Option(d.getProbability.toDouble)
                }
              )
          })
      },
      nodes = n.getNodes match {
        case null => None
        case _ =>
          Option(n.getNodes.asScala.map { n =>
            parseNode(n)
          })
      },
      embeddedModel = n.getEmbeddedModel match {
        case null => None
        case _ => Option(EmbeddedModel(
          output = n.getEmbeddedModel.getOutput match {
            case null => None
            case _ => Option(
              Output(
                extension = n.getEmbeddedModel.getOutput.getExtensions match {
                  case null => None
                  case _ =>
                    Option(n.getEmbeddedModel.getOutput.getExtensions.asScala.map { e =>
                      Extension(
                        extender = e.getExtender match {
                          case null => None
                          case _    => Option(e.getExtender)
                ...
                outputField =
                  n.getEmbeddedModel.getOutput.getOutputFields.asScala
                    .map {
                      o =>
                        OutputField(
                          name = o.getName.getValue,
          ...
          modelStats = n.getEmbeddedModel.getModelStats match {
            case null => None
            case _ =>
              Option(
                ModelStats(
                  univariateStats = n.getEmbeddedModel.getModelStats.getUnivariateStats match {
                    case null => None
                    case _ =>
                      Option(
                        n.getEmbeddedModel.getModelStats.getUnivariateStats.asScala
                          .map {
                            u =>
                              UnivariateStats(
                                field = u.getField.getValue,
                  ...
                  multivariateStats =
                    n.getEmbeddedModel.getModelStats.getMultivariateStats match {
                      case null => None
                      case _ =>
                        Option(
                          n.getEmbeddedModel.getModelStats.getMultivariateStats.asScala
                            .map {
                              m =>
                                MultivariateStats(
                                  targetCategory = m.getTargetCategory match {
          ...
          targets = n.getEmbeddedModel.getTargets match {
            case null => None
            case _ =>
              Option(n.getEmbeddedModel.getTargets.asScala.map {
                t =>
                  Targets(
                    field = t.getField match {
                      case null => None
          ...
          localTransformations = n.getEmbeddedModel.getLocalTransformations match {
            case null => None
            case _ =>
              Option(
                LocalTransformation(
                  extension = n.getEmbeddedModel.getLocalTransformations.getExtensions match {
                    case null => None
                    case _ =>
                      Option(
                        n.getEmbeddedModel.getLocalTransformations.getExtensions.asScala
                          .map { e =>
                            Extension(
                              extender = e.getExtender match {
                                case null => None
                  ...
                  derivedFields =
                    n.getEmbeddedModel.getLocalTransformations.getDerivedFields match {
                      case null => None
                      case _ =>
                        Option(
                          n.getEmbeddedModel.getLocalTransformations.getDerivedFields.asScala
                            .map { d =>
                              DerivedField(
                                name = d.getName match {
                                  case null => None
          ...
          modelName = n.getEmbeddedModel.getModelName match {
            case null => None
            case _    => Option(n.getEmbeddedModel.getModelName)
          },
          functionName = n.getEmbeddedModel.getMiningFunction.value()
        ))
      },
      id = n.getId match {
        case null => None
        case _    => Option(n.getId)
      },
      score = n.getScore match {
        case null => None
        case _    => Option(n.getScore)
      },
      recordCount = n.getRecordCount match {
        case null => None
        case _    => Option(n.getRecordCount.toDouble)
      },
      defaultChild = n.getDefaultChild match {
        case null => None
        case _    => Option(n.getDefaultChild)
      }
    )
  }
