...
trait RuleSet extends RuleSetModel {

  /** Parses provided pMML file as an Rule Set Model
        ...
        ruleSectionMethod = model.getRuleSet.getRuleSelectionMethods.asScala.map {
          m => RuleSelectionMethod(
            extension = m.getExtensions match {
              case null => None
              case _ =>
                Option(m.getExtensions.asScala.map {
                  e =>
                    Extension(
                      extender = e.getExtender match {
                        case null => None
        ...
        scoreDistribution = model.getRuleSet.getScoreDistributions match {
          ...
          case _ => Option(model.getRuleSet.getScoreDistributions.asScala.map {
            d => ScoreDistribution(
              ...
              value = d.getValue,
              recordCount = d.getRecordCount.toInt,
        ...
        rules = model.getRuleSet.getRules.asScala.map {
          rule => Rule(
            simpleRule = ruleType(rule) match {
              case "simple" =>
                parseSimpleRule(rule.asInstanceOf[org.dmg.pmml.rule_set.SimpleRule])
              case _ => None
            },
            compoundRule = ruleType(rule) match {
              case "compound" => parseCompoundRule(rule.asInstanceOf[org.dmg.pmml.rule_set.CompoundRule])
              case _ => None
            }
          )
        },
  ...
  /** Returns an string response ("simple" | "compound") of ruleType
    *
    * @param r a Simple rule
    * @return String
    */
  def ruleType(r: org.dmg.pmml.rule_set.Rule): String = r match {
    case _: org.dmg.pmml.rule_set.SimpleRule => "simple"
    case _: org.dmg.pmml.rule_set.CompoundRule => "compound"
  }


  /** Returns an Option[SimpleRule] object
    *
    * @param r a Simple rule
    * @return Option[SimpleRule]
    */
  def parseSimpleRule(r: org.dmg.pmml.rule_set.SimpleRule): Option[SimpleRule] = {
    Option(SimpleRule(
      extension = r.getExtensions match {
        case null => None
        case _ => Option(r.getExtensions.asScala.map {
          e =>
            Extension(
              extender = e.getExtender match {
                case null => None
      ...
      scoreDistribution = r.getScoreDistributions match {
        case null => None
        case _ => Option(r.getScoreDistributions.asScala.map {
          d => ScoreDistribution(
            extension = d.getExtensions match {
              case null => None
              case _ => Option(d.getExtensions.asScala.map {
                e =>
                  Extension(
                    extender = e.getExtender match {
                      case null => None
      ...
      id = r.getId match {
        case null => None
        case _ => Option(r.getId)
      },
      score = r.getScore,
      recordCount = r.getRecordCount match {
        case null => None
        case _ => Option(r.getRecordCount.toDouble)
      },
      nbCorrect = r.getNbCorrect match {
        case null => None
        case _ => Option(r.getNbCorrect.toDouble)
      },
      confidence = r.getConfidence match {
        case null => None
        case _ => Option(r.getConfidence.toDouble)
      },
      weight = r.getWeight.toDouble
    ))
  }

  /** Returns an Option[CompoundRule] object, recursively
    *
    * @param r a Compound rule
    * @return Option[CompoundRule]
    */

  def parseCompoundRule(r: org.dmg.pmml.rule_set.CompoundRule): Option[CompoundRule] = {
    Option(CompoundRule(
      extension = r.getExtensions match {
        case null => None
        case _ => Option(r.getExtensions.asScala.map {
