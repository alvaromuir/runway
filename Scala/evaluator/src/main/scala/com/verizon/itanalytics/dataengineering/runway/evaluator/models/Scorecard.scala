...
import com.verizon.itanalytics.dataengineering.runway.evaluator.models.elements.ScoreCardModel
...
trait ScoreCard extends ScoreCardModel {
  ...
  def parseScoreCardModel(pMML: PMML): ScoreCardModel = {
    val model = pMML
      .getModels
      .get(0)
      .asInstanceOf[org.dmg.pmml.scorecard.Scorecard]
    ...
    ScoreCardModel(
      ...
      miningSchema = MiningSchema(miningFields = model.getMiningSchema.getMiningFields match {
          case null => None
          case _ =>
            Option(
      ...
      characteristics = model.getCharacteristics.asScala.map {
        c => Characteristic(
          ...
          attribute = c.getAttributes.asScala.map {
            a => Attribute(
              extension = a.getExtensions match {
                case null => None
                case _ =>
                  Option(a.getExtensions.asScala.map {
                    e =>
                      Extension(
                        extender = e.getExtender match {
                          case null => None
              ...
              complexPartialScore = a.getComplexPartialScore match {
                case null => None
                case _ => Option(ComplexPartialScore(
                  extension = a.getComplexPartialScore.getExtensions match {
                    case null => None
                    case _ =>
                      Option(a.getComplexPartialScore.getExtensions.asScala.map {
                        e =>
                          Extension(
                            extender = e.getExtender match {
                              case null => None
                  ...
                  expression = a.getComplexPartialScore.getExpression.toString
                ))
              },
              reasonCode = a.getReasonCode,
              partialScore = a.getPartialScore match {
                case null => None
                case _ => Option(a.getPartialScore.toDouble)
              }
            )
          },
          name = c.getName match {
            case null => None
            case _ => Option(c.getName.getValue)
          },
          reasonCode = c.getReasonCode,
          baselineScore = c.getBaselineScore.toDouble
        )
      },
      ...
      initialScore = model.getInitialScore.toInt,
      useReasonCodes = Option(model.isUseReasonCodes),
      reasonCodeAlgorithm = model.getReasonCodeAlgorithm.value(),
      baselineScore = model.getBaselineScore.toDouble,
      baselineMethod = model.getBaselineMethod.value(),
